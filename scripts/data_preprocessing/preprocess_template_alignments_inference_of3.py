...
2. disambiguates query-template residue correspondences and precomputes them into a more accessible format
...
2. iterate over inference query set entries and for each entry:
    3. iterate over chains in entry:
        4. if not already preprocessed, iterate over alignment rows:
            ...
            7. compute precache from template structure if not available and precache_mode = "greedy"
...
outputs:
- template cache npz files for each unique chain in each query containing row idx and q-t residue index maps
- inference query set with updated template paths pointing to the npz template cache files
