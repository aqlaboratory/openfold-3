...
class TestDiffusionConditioning(unittest.TestCase):
    ...
    def test_without_n_sample_channel(self):
        ...
        t = sigma_data * torch.exp(-1.2 + 1.5 * torch.randn(batch_size, device=si_trunk.device))
    ...
    def test_with_different_schedule(self):
        ...
        t = sigma_data * torch.exp(-1.2 + 1.5 * torch.randn((batch_size, n_sample), device=si_trunk.device))
    ...
    def test_with_same_schedule(self):
        ...
        t = sigma_data * torch.exp(-1.2 + 1.5 * torch.randn((1, 1), device=si_trunk.device))
