...
def write_results(decoy, af_result, prot_native=None, pdb_native=None, mismatch=False):
    ...
    if prot_native is None:
        ...
        rms_out = jnp_rmsd(
            jnp.asarray(prot_native.atom_positions[:, 1, :]),
            jnp.asarray(af_result["unrelaxed_protein"].atom_positions[:, 1, :]),
        )
...
for i, d in enumerate(decoy_list):
    decoy = Decoy(
        target=d[0],
        decoy_id=d[1],
        decoy_path=args.decoy_dir + "decoys/" + d[0] + "/" + d[1],
        ...
        if n + "_native" not in finished_decoys:
            decoy_dict[n].insert(
                ...
                Decoy(
                    target=n,
                    decoy_id="native.pdb",
                    decoy_path=args.decoy_dir + "natives/" + n + ".pdb",
                    rmsd=0,
                    rosettascore=-1,
                    gdt_ts=1,
                    tmscore=1,
