        ...
        Args:
            batch:
                Input feature dictionary. Features used in this function:
                    ...
                    - "atom_to_token_index": [*, N_atom, N_token] one-hot encoding of token index per atom
            ql:
...
class RefAtomFeatureEmbedder(nn.Module):
    ...
    def __init__(
        ...
        """
        super(RefAtomFeatureEmbedder, self).__init__()
    ...
    def forward(
        ...
        Args: 
            batch:
                Input feature dictionary. Features used in this function:
                    - "ref_pos":  [*, N_atom, 3] atom positions in the reference conformer
                    ...
                    - "ref_atom_name_chars": [*, N_atom, 4, 64] one-hot encoding of unicode 
                        integers representing unique atom names in the reference conformer
                    - "ref_space_uid": [*, n_atom,] numerical encoding of the chain id and 
                        residue index in the reference conformer
        Returns: 
            cl:
                [*, N_atom, c_atom] Atom single conditioning
            plm:
        ...
        ], dim = -1)) #CONFIRM THIS FORMAT ONCE DATALOADER/FEATURIZER DONE
        ...
        vlm = (batch['ref_space_uid'].unsqueeze(-2) == batch['ref_space_uid'].unsqueeze(-1)).to(dlm.dtype)
        plm = self.linear_ref_offset(dlm) * vlm.unsqueeze(-1)

        # Embed pairwise inverse squared distances
        # [*, N_atom, N_atom, c_atom_pair]
        inv_sq_dists = 1. / (1 + torch.sum(dlm ** 2, dim=-1, keepdim=True))
...
class NoisyPositionEmbedder(nn.Module):
    ...
    def forward(
        ...
        Returns: 
            cl:
                [*, N_atom, c_atom] Atom single conditioning with trunk single representation embedded
            plm:
                [*, N_atom, N_atom, c_atom_pair] Atom pair conditioning with trunk pair representation embedded
            ql:
                [*, N_atom, c_atom] Atom single representation with noisy coordinate projection 
        """
        # Broadcast trunk single representation into atom single conditioning
        # [*, N_atom, c_atom]
        sl_trunk = torch.einsum('...ic,...li->...lc', si_trunk, batch['atom_to_token_index'])
        cl = cl + self.linear_s(self.layer_norm_s(sl_trunk))

        # Broadcast trunk pair representation into atom pair conditioning
        # [*, N_atom, N_atom, c_atom_pair]
        zlj_trunk = torch.einsum('...ijc,...li->...ljc', zij_trunk, batch['atom_to_token_index'])
        zlm_trunk = torch.einsum('...ljc,...mj->...lmc', zlj_trunk, batch['atom_to_token_index'])
...
class AtomAttentionEncoder(nn.Module):
    ...
    def __init__(
        ...
        self.ref_atom_feature_embedder = RefAtomFeatureEmbedder(c_atom_ref=c_atom_ref,
                                                                c_atom=c_atom,
                                                                c_atom_pair=c_atom_pair)

        if add_noisy_pos:
            self.noisy_position_embedder = NoisyPositionEmbedder(c_s=c_s,
                                                                 c_z=c_z,
                                                                 c_atom=c_atom,
                                                                 c_atom_pair=c_atom_pair)
        ...
        self.linear_m = Linear(c_atom, c_atom_pair, bias=False, init="relu") # TODO: check initialization
    ...
    def forward(
        ...
        # ql: [*, N_atom, c_atom]
        if rl is not None:
            cl, plm, ql = self.noisy_position_embedder(batch=batch,
                                                       cl=cl,
                                                       plm=plm,
                                                       ql=ql,
                                                       si_trunk=si_trunk,
        ...
        # Add the combined single conditioning to the pair representation (line 13 - 14)
        # [*, N_atom, N_atom, c_atom_pair]
        plm = plm + self.linear_l(self.relu(cl.unsqueeze(-3))) + self.linear_m(self.relu(cl.unsqueeze(-2)))
        plm = plm + self.pair_mlp(plm)
        
        # Cross attention transformer (line 15)
        # [*, N_atom, c_atom]
        ql = self.atom_transformer(batch=batch,
                                   ql=ql,
                                   cl=cl,
                                   plm=plm)
        ...
        # [*, N_token, c_token] 
        ai = torch.einsum('...lc,...il->...ic', self.linear_q(ql), token_to_atom_index) / torch.sum(token_to_atom_index, dim=-1, keepdim=True) # TODO: check this
...
class AtomAttentionDecoder(nn.Module):
    ...
    def forward(
        ...
        # [*, N_atom, c_atom]
        ql = ql + torch.einsum('...ic,...li->...lc', self.linear_q_in(ai), batch['atom_to_token_index'])
