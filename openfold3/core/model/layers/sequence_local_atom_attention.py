...
class AtomTransformer(nn.Module):
    ...
    def forward(self, 
        ...
        # [N_atom, N_atom]
        blm = torch.sum(torch.logical_and(row_condition.unsqueeze(1),
                                          col_condition.unsqueeze(0)).to(ql.dtype), dim=-1) * 1e10 - 1e10
...
class AtomFeatureEmbedder(nn.Module):
    ...
    def forward(
        ...
        # [*, N_atom, N_atom, 3] -> [*, N_atom, N_atom, C_atom_pair]
        inv_dists = self.linear_inv_dists(torch.pow(1.0 + torch.norm(dlm, dim=-1).unsqueeze(-1), -1.))
