...
def crop_atom_array(
    ...
    Note: also adds a crop_mask and token_position annotation to the input atom array.
    Here, token_position indexes the position of each token in the crop, which is needed
    for mapping the crop to the alignments and templates.

    Note: no cropping is performed if the apply_crop argument is set to false or the
    input atom array fits in the budget.
    ...
    Args:
        ...
        crop_config (dict):
            Crop configuration dictionary, containing the following keys: - token_budget
            (int): Number of tokens to sample. - crop_weights (dict): Weights of
    ...
    # Otherwise crop
        ...
        crop_function, crop_function_argnames = sample_crop_strategy(
            crop_config["crop_weights"]
        )
        crop_input = {
            "atom_array": atom_array,
