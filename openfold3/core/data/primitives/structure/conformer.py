...
def resolve_and_format_fallback_conformer(
    ...
    AnnotatedMol,
    Literal["default", "random_init", "use_fallback"],
    Literal["rdkit", "ccd-ideal", "ccd-model", "all-nan"],
]:
    ...
    Returns:
        ...
        fallback_conformer_source:
            The source of the fallback conformer coordinates:
                - "rdkit": Generated by RDKit's ETKDGv3 strategy
                - "ccd-ideal": Ideal coordinates from the CCD entry
                - "ccd-model": Model coordinates from the CCD entry
                - "all-nan": No conformer available (all-NaN coordinates)
    """
    ...
    try:
        ...
        fallback_conformer_source = "rdkit"
    except ConformerGenerationError:
        strategy = "use_fallback"
        # Try to use first stored conformer
        try:
            conf = next(mol.GetConformers())
            # Determine source from the conformer name set during mol construction
            conf_name = conf.GetProp("name") if conf.HasProp("name") else None
            if conf_name == "Ideal":
                fallback_conformer_source = "ccd-ideal"
            elif conf_name == "Model":
                fallback_conformer_source = "ccd-model"
            else:
                raise ValueError(
                    f"Unexpected conformer name: {conf_name!r}. Expected one "
                    "of 'Ideal' or 'Model'."
                )
        # If no stored conformer, use all-NaN conformer
        except StopIteration:
            conf = get_allnan_conformer(mol)
            fallback_conformer_source = "all-nan"
    ...
    return mol, strategy, fallback_conformer_source
