...
def tokenize_atomarray(atomarray: AtomArray):
    ...
    residue_token_start_ids = np.unique(struc.get_residue_starts_for(atomarray, standard_residue_atom_ids))
    
    # Get atom-token ids
    atom_token_start_ids = atomidx[is_standard_residue_atom == False]

    # Combine for all token start ids
    all_token_start_ids = np.sort(np.concatenate([residue_token_start_ids, atom_token_start_ids]))
    ...
    af3_token_center_atoms[is_standard_residue_atom] = np.isin(atomarray[is_standard_residue_atom].atom_name, TOKEN_CENTER_ATOM_NAMES)
