...
def crop_spatial(
    ...
    atom_array.set_annotation("af3_crop_mask", np.isin(atom_array.af3_token_id, token_center_atoms[nearest_token_center_atom_ids].af3_token_id))
    return None
...
def crop_spatial_interface(
    ...
    token_budget: int,
    generator: Generator,
    preferred_chain_or_interface: Optional[Union[int, tuple[int, int]]] = None) -> None:
    # Subset token center atoms to those in the preferred chain/interface if provided
    token_center_atom_ids = atom_array[atom_array.af3_token_center_atom].af3_atom_id

    n = len(token_center_atom_ids)
    arr_repeated = np.repeat(token_center_atom_ids, n)
    arr_tiled = np.tile(token_center_atom_ids, n)
    pairwise_array = np.column_stack((arr_repeated, arr_tiled))

    struc.distance(atom_array[pairwise_array[:, 0]], atom_array[pairwise_array[:, 1]])
