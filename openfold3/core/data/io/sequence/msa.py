...
def parse_mmseqs_a3m(msa_string: str) -> dict[str, MsaArray]:
    """
    Parses multisample a3m files generated by mmseqs2
    Args:
        msa_string (str): single string containing whole a3m file
    Returns:
        dict[str, MsaArray]: maps mmseqs internal chain id to MsaArray
    """
    mmseqs_chainid2msa = {}
    for msa_piece in msa_string.split("\0"):
        if msa_piece:  ## for handling end of file
            # get chain_id
            mmseqs_chainid = msa_piece.split("\n")[0].strip(">")
            msa = parse_a3m(msa_piece)
            mmseqs_chainid2msa[mmseqs_chainid] = msa
    return mmseqs_chainid2msa
