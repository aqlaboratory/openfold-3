...
class ParamType(Enum):
    ...
    LinearWeightMultimer = partial(
        lambda w: w.unsqueeze(-1) if len(w.shape) == 1 else w.reshape(w.shape[0], -1).transpose(-1, -2)
    )
...
def convert_deprecated_v2_keys(state_dict):
    """Update older OpenFold model weight names to match the current model code.
    TODO: Combine with v1 version
    """
...
def import_openfold_weights_(model, state_dict):
    ...
    try:
        ...
        converted_state_dict = convert_deprecated_v2_keys(convert_deprecated_v1_keys(state_dict))
