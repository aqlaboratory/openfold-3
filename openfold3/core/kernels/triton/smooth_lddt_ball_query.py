    ...
    def _ball_query_forward_kernel(
        ...
        for j0 in tl.range(0, len2, BLOCK_J):
            ...
            if n_hits > 0 and count < K:
                ...
                if PRED_IS_BF16:
                    px = tl.load(predx_ptr + j_ptr, mask=take, other=0.0).to(
                        tl.float32
                    )
                    py = tl.load(predy_ptr + j_ptr, mask=take, other=0.0).to(
                        tl.float32
                    )
                    pz = tl.load(predz_ptr + j_ptr, mask=take, other=0.0).to(
                        tl.float32
                    )
                else:
                    ...
                    for b in tl.static_range(BLOCK_J):
                        ...
                        if ib:
                            ...
                            if hit_ord > n_take:
                                ...
                                s_pdz = pi_z - s_pz
                                s_d2p = (
                                    s_pdx * s_pdx + s_pdy * s_pdy + s_pdz * s_pdz
                                )
                                rng = _hash_rng(rng)
