...
from openfold3.core.kernels.smooth_lddt import (
    ball_query_smooth_lddt_loss,
    is_smooth_lddt_kernel_available,
)
...
def smooth_lddt_loss_ball_query(
    ...
    """
    Compute smooth lDDT with a top-k ball-query backend.

    Uses reservoir sampling for unbiased random neighbor selection when more
    than ``top_k`` atoms qualify. Pass the training step as ``seed`` for
    reproducibility with different random subsets each step.

    This is exact relative to ``smooth_lddt_loss`` when ``top_k`` covers all
    atoms inside the smooth lDDT cutoff for each query atom.
    """
    x_gt = batch["ground_truth"]["atom_positions"]
    atom_mask_gt = batch["ground_truth"]["atom_resolved_mask"]
    is_nucleotide = broadcast_token_feat_to_atoms(
...
def get_smooth_lddt_loss_fn(smooth_lddt_backend: str) -> Callable:
    if smooth_lddt_backend == "dense":
        return smooth_lddt_loss

    if smooth_lddt_backend == "ball_query":
        if not is_smooth_lddt_kernel_available():
            raise RuntimeError("Ball-query smooth lDDT backend is not available")
...
def diffusion_loss(
    ...
    smooth_lddt_top_k: int | None = None,
    **kwargs,
) -> [torch.Tensor, dict]:
    """
    ...
    Args:
        ...
        smooth_lddt_top_k:
            Number of nearest non-self atoms per query atom for ball-query smooth lDDT.
    Returns:
        mean_loss:
            Diffusion loss
        loss_breakdown:
    ...
    smooth_lddt_loss_fn = get_smooth_lddt_loss_fn(smooth_lddt_backend)
    ...
    if smooth_lddt_backend == "ball_query":
        smooth_lddt_kwargs["top_k"] = smooth_lddt_top_k
    ...
    if chunk_size is None:
        ...
        if smooth_lddt_weight.any():
            l_smooth_lddt = smooth_lddt_loss_fn(x=x, **smooth_lddt_kwargs)
        ...
        if smooth_lddt_weight.any():
            l_smooth_lddt = run_low_mem_loss_fn(
                loss_fn=smooth_lddt_loss_fn,
                x=x,
                kwargs=smooth_lddt_kwargs,
                chunk_size=chunk_size,
            )
            loss_breakdown["smooth_lddt"] = l_smooth_lddt.detach().clone().mean(dim=-1)
