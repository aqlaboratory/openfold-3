...
from openfold3.tests.utils.compare_utils import assert_differences_along_last_dim, assert_max_abs_diff_small
...
class TestTemplateEmbedderAllAtom(unittest.TestCase):
    ...
    def test_multimer_masking(self):
        ...
        c_in = of3_config.architecture.template.template_pair_embedder.c_in
        template_pair_embedder_conf = of3_config.architecture.template.template_pair_embedder
        ...
        for it, len_first in enumerate(len_firsts):
            ...
            repeated_first = monomer_embedded[it,0, 0, 0, :].squeeze(0).tile([n_templ, len_first-1, len_first-1, 1])
            repeated_last = monomer_embedded[it,0,-1,-1, :].squeeze(0).tile([n_templ, n_token-len_first-1, n_token-len_first-1, 1])
            top_left = monomer_embedded[it, :, 1:len_first, 1:len_first, :].squeeze()
            bottom_right = monomer_embedded[it, :, len_first:-1, len_first:-1, :].squeeze()
            ...
            repeated_off_top_right = monomer_embedded[it,0, len_first, len_first-1, :].squeeze(0).tile([n_templ, len_first, n_token-len_first, 1])
            repeated_off_bottom_left = monomer_embedded[it,0, len_first, len_first-1, :].squeeze(0).tile([n_templ, n_token-len_first, len_first, 1])
        ...
        for it, len_first in enumerate(len_firsts):
            ...
            repeated_first = multimer_embedded[it,0, 0, 0, :].squeeze(0).tile([n_templ, len_first-1, len_first-1, 1])
            repeated_last = multimer_embedded[it,0,-1,-1, :].squeeze(0).tile([n_templ, n_token-len_first-1, n_token-len_first-1, 1])
            top_left = multimer_embedded[it, :, 1:len_first, 1:len_first, :].squeeze()
            bottom_right = multimer_embedded[it, :, len_first:-1, len_first:-1, :].squeeze()
            assert_differences_along_last_dim(repeated_first, top_left)
            assert_differences_along_last_dim(repeated_last , bottom_right)

            # off the monomers is unmasked and always different again! 
            repeated_off_top_right = multimer_embedded[it,0, len_first, len_first-1, :].squeeze(0).tile([n_templ, len_first, n_token-len_first,1]) 
            repeated_off_bottom_left = multimer_embedded[it,0, len_first, len_first-1, :].squeeze(0).tile([n_templ, n_token-len_first, len_first-1, 1])
            top_right = multimer_embedded[it,:, :len_first,   len_first:, :].squeeze()
            bottom_left = multimer_embedded[it,:, len_first:, :len_first-1, :].squeeze()
