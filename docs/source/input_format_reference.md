...
Optional query-level fields include:
  - `covalent_bonds` *(list, optional, default = null)*
    - Explicit bonds between named atoms. See {ref}`query-covalent-bonds`.
...
Specify each bond using `atom1` and `atom2`, each a triple of
`[chain_id, residue_id, atom_name]`. Residue IDs start at 1; a single-component
ligand has residue ID 1. Use CCD atom names for CCD components. For SMILES
ligands, use the atom names generated by OF3's reference-molecule builder.

```json
"covalent_bonds": [
  {"atom1": ["A", 1, "SG"], "atom2": ["B", 1, "SG"]}
]
```

Endpoints must each identify exactly one modeled atom. Self-bonds are rejected;
repeated bonds are ignored and existing bond orders are preserved. New bonds
are single bonds and enter OF3's existing tokenization and `token_bonds`
